...
@section('main')
    <div class="container" style="margin-top: 20px;">
        <div class="jumbotron">
            <p class="text text-info">You have successfully authenticated and can now use this to display {{ $action }} viewer(s) in your channel using chatbots.</p>

            <h3>Streamlabs Chatbot (formerly Ankhbot):</h3>
            <p class="text text-muted">Add a command that includes the part below and it will be replaced with {{ $action }} viewer(s):</p>
            <pre>$readapi({{ $route }}/$mychannel)</pre>
            <p class="text text-muted">For example, if you want to show @if ($action == 'latest')your last @elseif ($action == 'random')a random @endif viewer of your stream:</p>
            <pre>{{ ucfirst($action) }} viewer is: {{ $channel }}</pre>
            <p class="text text-muted">You would put this in the command response:</p>
            <pre>{{ ucfirst($action) }} viewer is: $readapi({{ $route }}/$mychannel)</pre>
            ...
            <pre>{{ ucfirst($action) }} viewer is: {{ $channel }}</pre>
            <p class="text text-muted">You would put this in the command response:</p>
            <pre>{{ ucfirst($action) }} viewer is: $(urlfetch {{ $route }})</pre>
        ...
        <div class="row">
            ...
            <table id="qs-body" class="table table-bordered table-striped table-hover">
                ...
                <tbody>
                    <tr>
                        <th>count</th>
                        <td>The amount of viewers to retrieve - Default: <code>1</code>.</td>
                        <td><code>int</code></td>
                    </tr>
                    <tr>
                        <th>field</th>
                    ...
                    <tr>
                        <th>separator</th>
                        <td>What character(s) should separate each viewer value from each other. Default: <code>, </code>.</td>
                        <td><code>string</code></td>
                    </tr>
                </tbody>
            </table>
